...
describe('Images demo endpoint contains expected information', () => {
	...
	it(`checks images list can be accessed`, () => {
		const jsonSchemaImages = {
			type: 'object',
			properties: {
				mediaType: {
					type: 'string',
			...
			required: ['mediaType', 'files'],
		};
		cy.request('/api/images/list').then((response) => {
			expect(response.status).to.eq(200);
			expect(response.body).to.be.jsonSchema(jsonSchemaImages);
		});
	});
});
...
describe('Videos demo endpoint contains expected information', () => {
	...
	it(`checks videos list can be accessed`, () => {
		const jsonSchemaVideos = {
			type: 'object',
			properties: {
				mediaType: {
					type: 'string',
		...
		cy.request('/api/videos/list').then((response) => {
			expect(response.status).to.eq(200);
			expect(response.body).to.be.jsonSchema(jsonSchemaVideos);
		});
	});
});
