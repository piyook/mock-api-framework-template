...
// Static resource
server.tool('get_code_format', 'returns the format to be used for API endpoint code generation to be used with add_localMockAPIEndpoint tool ', {}, () => ({
    content: [
        {
            type: 'text',
...
server.tool('get_localMockAPIEndpoints', 'returns all available API endpoints on local mock API server on localhost', {}, async () => ({
    content: [
        {
            type: 'text',
...
server.tool('add_localMockAPIEndpoint', `adds a new API endpoint to the local mock API server on localhost 
	Args:
