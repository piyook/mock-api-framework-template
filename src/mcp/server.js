...
import { startMockServer, stopMockServer, rebuildMockServer, } from './helpers/control-mock-server.js';
...
server.tool('create_new_api_endpoint', `create new api endpoint for the local mock API server using the supplied standard code format as a basis for the new api code. 
	Args:
...
server.tool('manage_local_mock_api_server', `Manage the local mock server on localhost running in Docker (Docker must be installed and running). 	
	Args:
