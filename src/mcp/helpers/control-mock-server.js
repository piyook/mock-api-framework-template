...
const manageServer = (command, mode, PORT) => {
    shell.exec(command, {
        async: false,
        silent: true,
    });
...
const startMockServer = async (PORT) => {
    return manageServer(`cd "${__dirname}" && docker-compose up -d`, 'start', PORT);
};
const stopMockServer = async (PORT) => {
    return manageServer(`cd "${__dirname}" && docker-compose down --remove-orphans --volumes --timeout 10`, 'stop', PORT);
};
const rebuildMockServer = async (PORT) => {
    return manageServer(`cd "${__dirname}" && docker-compose down --remove-orphans --volumes --timeout 10 &&docker-compose up -d --build --force-recreate`, 'rebuild', PORT);
};
